...
# Один постоянный asyncio loop для aiogram.
# Иначе asyncio.run(...) на каждом webhook-запросе закрывает loop
# и появляется RuntimeError: Event loop is closed.
...
def webhook():
    try:
        ...
        # Telegram webhook должен получить ответ сразу.
        # Обработку aiogram запускаем в фоне на постоянном event loop.
        schedule_async(dp.feed_update(bot, update))
        return "ok", 200
    except Exception as e:
