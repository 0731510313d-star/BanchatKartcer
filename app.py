    ...
    if total < MIN_VOTES:
        ...
        if percent >= BAN_PERCENT:
            try:
                ...
                text = (
                    "🔨 <b>ПОЛЬЗОВАТЕЛЬ ЗАБАНЕН ПО ИТОГАМ ГОЛОСОВАНИЯ</b>\n\n"
                    f"👤 {target_name}\n"
                    f"📝 Причина: {reason}\n\n"
                    f"🔨 За бан: {yes}\n"
                    f"🛡 Против: {no}\n"
                    f"📊 За бан: {percent:.0f}%\n\n"
                    "👮 Администратор может отменить решение и снять бан."
                )
                restore_keyboard = {
                    "inline_keyboard": [
                        [
                            {
                                "text": "↩️ ВЕРНУТЬ УЧАСТНИКА",
                                "callback_data": f"restore:{vote_id}",
                            }
                        ]
                    ]
                }
                send_message(chat_id, text, reply_markup=restore_keyboard)

            except Exception as e:
                print("AUTO BAN ERROR:", repr(e), flush=True)
                with db_lock:
                    db.execute(
                        "UPDATE votes SET decision_status='ban_failed' WHERE id=?",
                        (vote_id,),
                    )
                    db.commit()
                ...
                send_message(
                    chat_id,
                    "⚠️ Голосование прошло, но бот не смог забанить пользователя.\n\n"
                    "Проверь, что у бота есть право блокировать участников."
                )
            ...
            text = (
                ...
                f"🛡 Против: {no}\n"
                f"📊 За бан: {percent:.0f}%\n\n"
                f"Для бана нужно минимум {BAN_PERCENT}% голосов «ЗА»."
            )
            send_message(chat_id, text)
...
def handle_message(message):
    ...
    if base_command == "/start":
        send_message(
            ...
            "Если голосование проходит, бот автоматически банит участника. Администратор может отменить бан кнопкой «Вернуть участника».",
    ...
    if data.startswith("restore:"):
        ...
        try:
            if not is_admin(chat_id, user_id):
                answer_callback(
                    callback_id,
                    "⛔ Вернуть участника может только администратор.",
                    True,
                )
                return
        except Exception as e:
            print("RESTORE ADMIN CHECK ERROR:", repr(e), flush=True)
            answer_callback(callback_id, "Не удалось проверить права администратора.", True)
            return

        with db_lock:
            row = db.execute(
                "SELECT target_id,target_name,reason,decision_status "
                "FROM votes WHERE id=?",
                (vote_id,),
            ).fetchone()
        ...
        target_id, target_name, reason, decision_status = row

        if decision_status == "restored":
            answer_callback(callback_id, "Участник уже разбанен.", True)
            return

        if decision_status != "banned":
            answer_callback(callback_id, "Этот бан уже нельзя отменить этой кнопкой.", True)
            return
        ...
        try:
            ...
            with db_lock:
                db.execute(
                    "UPDATE votes SET decision_status='restored' WHERE id=?",
                    (vote_id,),
                )
                db.commit()
            ...
            try:
                ...
                print("RESTORE BUTTON REMOVE ERROR:", repr(e), flush=True)

            answer_callback(callback_id, "Бан снят.", False)
            send_message(
                chat_id,
                "↩️ <b>АДМИНИСТРАТОР ОТМЕНИЛ БАН</b>\n\n"
                f"👤 {target_name}\n"
                "Пользователь разбанен и может снова вступить в группу."
            )

        except Exception as e:
            print("RESTORE ERROR:", repr(e), flush=True)
            answer_callback(
                callback_id,
                "⚠️ Не удалось снять бан. Проверь права бота.",
                True,
            )
        return
...
def set_webhook():
    try:
        ...
        return (
            ...
            "Automatic ban by vote: ON\n"
            "Admin restore after ban: ON\n"
            "Minimum votes: 15\n"
